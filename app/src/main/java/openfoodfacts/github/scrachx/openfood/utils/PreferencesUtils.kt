...
inline fun <T : Preference?> PreferenceScreen.requirePreference(key: String, action: T.() -> Unit) {
    requirePreference<T>(key).run(action)
}
...
fun Context.getCameraPreferences(mode: Int = Context.MODE_PRIVATE): SharedPreferences =
    getSharedPreferences("camera", mode)
