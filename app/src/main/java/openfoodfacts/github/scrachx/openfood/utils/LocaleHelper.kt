...
object LocaleHelper {
    ...
    fun find(availableLanguages: List<LanguageData>, language: String?): Int {
        if (language != null) {
            availableLanguages.withIndex().forEach { (i, availableLanguage) ->
    ...
    fun getLanguageData(codes: Collection<String?>?, supported: Boolean): List<LanguageData> {
        val res: MutableList<LanguageData> = ArrayList()
        if (codes != null) {
            for (code in codes) {
                val languageData = getLanguageData(code, supported)
    ...
    @JvmStatic
    val locale: Locale
        get() = getLocale(OFFApplication.getInstance())
