...
fun <T> Observable<T>.log(streamName: String, message: String? = null): Observable<T> {
    ...
    return this
            .doOnNext {
                Log.d(streamName, "$msg: $it")
            }
            .doOnError {
                Log.w(streamName, "$msg [Error]: $it: ${it.message}")
            }
            .doOnComplete {
                Log.w(streamName, "$msg [Complete]")
            }
            .doOnSubscribe {
                Log.w(streamName, "$msg [Subscribed]")
            }
            .doOnDispose {
                Log.w(streamName, "$msg [Disposed]")
            }
}
